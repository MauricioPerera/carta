...
`urllib` (Ollama Cloud, LM Studio, …). During development the *orchestrator* role
was filled by several different agents (Claude, Perplexity, Antigravity),
delegating the actual coding to small models (e.g. `glm-5.2` + `kimi-k2.7-code`
on Ollama Cloud).
...
Carta is not on PyPI yet — install from a clone (`pip install -e .`).
...
carta init . --name my-project          # scaffolds the team + CCDD flow,
                                        #   including flows/example.flow.yaml
carta flow flows/example.flow.yaml --input SPEC.md
python -m pytest tests/frozen/ -q       # verify the result yourself
```

`flows/example.flow.yaml` is generated by `carta init` — run `init` first; it
does not exist in a bare checkout.
